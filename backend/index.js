...
const TrainerModel = require('./models/TrainerModel'); // Import TrainerModel
...
app.get('/trainers', async (req, res) => {
  try {
      ...
      res.json(trainers); // Send trainers as response
  } catch (error) {
      res.status(500).json({ error: 'Failed to fetch trainers' });
  }
});

// Route to update trainer details (including file upload)
app.put('/trainers/:id', upload.single('photo'), async (req, res) => {
  const { phone, specialization, experience } = req.body;
  const updates = { phone, specialization, experience };

  if (req.file) {
      updates.photo = req.file.path; // Save the uploaded file path (photo/pdf)
  }
  ...
  try {
      ...
      res.json(trainer); // Send updated trainer details
  } catch (error) {
      res.status(500).json({ error: 'Failed to update trainer details' });
  }
});
...
app.get('/members', async (req, res) => {
  try {
    const members = await Member.find(); // Fetch members from the Member collection
    res.json(members);
  } catch (error) {
...
app.post('/attendance', async (req, res) => {
  const attendanceData = req.body; // The attendance data sent from the frontend

  try {
    await Attendance.insertMany(attendanceData); // Insert the array of attendance records
...
// Get attendance records by date, populated with member names
app.get('/attendance', async (req, res) => {
  const { date } = req.query; // Get the date from query params

  try {
    const attendanceRecords = await Attendance.find({ date }) // Fetch attendance records for the specific date
      .populate('memberId', 'fullName') // Populate memberId with the member's full name
      .exec(); // Execute the query

    res.json(attendanceRecords); // Send the populated attendance records as a response
  } catch (error) {
...
app.post('/trainers', multer({ storage: multer.diskStorage({
  ...
  const { name, email, phone, experience, address } = req.body;
  const photo = req.file.path; // Path to uploaded photo

  try {
      const newTrainer = new TrainerModel({ name, email, phone, experience, photo, address });
...
// Update a trainer
app.put('/trainers/:id', multer({ storage: multer.diskStorage({
  destination: (req, file, cb) => cb(null, 'uploads/'),
  filename: (req, file, cb) => cb(null, Date.now() + '-' + file.originalname)
})}).single('photo'), async (req, res) => {
  const { name, email, phone, experience, address } = req.body;
  const updates = { name, email, phone, experience, address };

  if (req.file) {
      updates.photo = req.file.path; // Update with the new photo path if uploaded
  }

  try {
      const updatedTrainer = await TrainerModel.findByIdAndUpdate(req.params.id, updates, { new: true });
      if (!updatedTrainer) return res.status(404).json({ message: 'Trainer not found' });
      res.json(updatedTrainer);
  } catch (err) {
      console.error('Error updating trainer:', err);
